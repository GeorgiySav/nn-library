...
void launch(const Stream& s, const T* src, TensorView v, T* dst, int64_t n) {
  ...
  pack_kernel<T><<<grid, kBlock, 0, stream>>>(src, v, dst, n);
  NN_CUDA_CHECK_LAUNCH(stream);
}

}

void cuda_pack(const Stream& s, const float* src, TensorView v,
                       float* dst, int64_t n) {
  launch(s, src, v, dst, n);
}

void cuda_pack_i32(const Stream& s, const int32_t* src, TensorView v,
