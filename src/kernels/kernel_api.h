...
// destination. unpack is the one deliberate exception, since writing a
// result into a window of a larger dense tensor is what the backward of
// slice is, and it cannot be expressed as a dense output.
...
// scaled by weights[row], a plain [M] float array that is never itself a
// gradient target, the same role as labels.
...
// trusts it, the same division of labour as every other ops:: shape check.
...
// the elementwise family. one slot per arity rather than per op, since the
// op code selects the arithmetic inside the kernel, from elementwise_ops.h.
...
using BinaryFn            = void(const Stream& s, BinaryOp op,
                                    ...
                                    float* C, int64_t n);
// side 0 selects d/dA, side 1 selects d/dB, both at the broadcast shape.
...
// destination's strides, which is what the backward of slice and the forward
// of cat need. neither is a strided variant of anything, the strides are
// the whole point, so there is no sibling to pair them with.
