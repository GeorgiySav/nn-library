...
template<class T>
void copy_strided_impl(const T* src, TensorView v, T* dst, int64_t n) {
...
void naive_copy_strided(const Stream&, const float* src, TensorView v,
                        float* dst, int64_t n) {
  copy_strided_impl(src, v, dst, n);
}

void naive_copy_strided_i32(const Stream&, const int32_t* src, TensorView v,
