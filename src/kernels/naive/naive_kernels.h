...
// lda/ldb/ldc are leading dimensions
// the distance between consecutive rows of
// each operand as stored. They equal K/N/N only when the operand is dense.
void naive_gemm(const Stream& s, const float* A, const float* B, float* C, int M, int N, int K,
                int64_t lda, int64_t ldb, int64_t ldc, bool transA, bool transB);
...
void naive_softmax_ce(const Stream& s, const float* logits, const int32_t* labels, float* loss_out, float* probs, int M, int N, int64_t sz);
void naive_softmax_ce_backward(const Stream& s, const float* probs, const int32_t* labels, const float* g_loss, float* g_logits, int M, int N, int64_t sp);
...
void naive_fill_from(const Stream& s, const float* src, float* X, int64_t n);

void naive_relu_strided(const Stream& s, const float* X, TensorView v, float* Y, int64_t n);
void naive_relu_backward_strided(const Stream& s, const float* X, TensorView vx,
                                 const float* gY, TensorView vg, float* gX, int64_t n);
void naive_add_strided(const Stream& s, const float* A, TensorView va,
...
void naive_copy_strided(const Stream& s, const float* src, TensorView v, float* dst, int64_t n);
void naive_copy_strided_i32(const Stream& s, const int32_t* src, TensorView v, int32_t* dst, int64_t n);
void naive_copy_into_strided(const Stream& s, const float* src, float* dst, TensorView vdst, int64_t n);
}